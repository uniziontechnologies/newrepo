...
<?php
	...
	foreach ($ledger_dc as $i => $ledger)
	{
		...
		if ($current_entry_type['label']=="receipt" || $current_entry_type['label']=="payment" ) {

				if ($i==0) {

					if ($current_entry_type['bank_cash_ledger_restriction'] == '4')
						echo "<td>" . form_input_ledger('ledger_id[' . $i . ']', 1, '', $type = 'bankcash') . "</td>";
					else if ($current_entry_type['bank_cash_ledger_restriction'] == '5')
						echo "<td>" . form_input_ledger('ledger_id[' . $i . ']', 1, '', $type = 'nobankcash') . "</td>";
					else
						echo "<td>" . form_input_ledger('ledger_id[' . $i . ']', 1) . "</td>";
