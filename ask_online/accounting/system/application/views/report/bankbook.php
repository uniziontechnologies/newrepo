...
<?php
	...
	if ( ! $print_preview)
	{
		...
		// $ledger_id=1;
		$attr=array('id'=>'form');
		echo form_open('report/bankbook/' . $ledger_id,$attr);
		...
		echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";	
		echo "<div>";				
		echo form_input_ledger('ledger_id', $ledger_id,'','bankbook');
		echo " ";
	...
	if ( ! $print_preview)
	{
		...
		if ( ! $page_count)
			$page_count = "0";
		$config['base_url'] = site_url('report/bankbook/' . $ledger_id);
		$config['num_links'] = 10;
		...
		if ($ledger_id==0) {
					...
					foreach ($ledgerst_q->result() as $row )
					{
						...
						$ledger_id_array1[]=$row->id;

					}

					if (!empty($ledger_id_array1)) {
						$ledger_id_array1 = implode(',', $ledger_id_array1);

						$sql_mode = "in (".$ledger_id_array1.")";
					}
					else{
						$sql_mode = "";
					}

			$sql="SELECT * FROM `entries` JOIN  `entry_items` ON  `entries`.`id` =  `entry_items`.`entry_id` WHERE  `entry_items`.`ledger_id` $sql_mode AND `entries`.`date` >= '$search_from' AND `entries`.`date` <= '$search_to'";
		}
		else{
		...
		if ( ! $print_preview) {
			...
			if ($ledger_id != 0) {
					...
					if (!empty($ledger_id_array)) {
					    $ledger_id_string = implode(',', $ledger_id_array);
					} else {
					    $ledger_id_string = '0';
					}

			        $sql = "SELECT entries.id AS entries_id, entries.number AS entries_number, entries.date AS entries_date, entries.narration AS entries_narration, entries.entry_type AS entries_entry_type, entry_items.amount AS entry_items_amount, entry_items.dc AS entry_items_dc FROM  `entries` JOIN  `entry_items` ON  `entries`.`id` =  `entry_items`.`entry_id` WHERE `entry_items`.`ledger_id` in ($ledger_id_string) AND `entries`.`date` >=  '$search_from' AND  `entries`.`date` <=  '$search_to' ORDER BY  `entries`.`date` ASC ,  `entries`.`number` ASC LIMIT $page_count,$pagination_counter";
			        // print_r($sql);
