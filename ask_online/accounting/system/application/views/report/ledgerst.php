...
<?php
	...
	if ( ! $print_preview)
	{
		...
		$attr=array('id'=>'form');
		echo form_open('report/ledgerst/' . $ledger_id,$attr);
	...
	if ($ledger_id != "")
	{
		...
		if ($page_count <= 0)
		{
			...
			if ($optype=="D")
			{
				echo "<tr class=\"tr-balance\"><td colspan=4>Opening Balance</td><td>".$dr_sum."</td><td>".$cr_sum."</td><td>" . convert_opening($opbalance,$optype) . "</td></tr>";
				$cur_balance = float_ops($opbalance, $cur_balance, '+');
			} else {

				echo "<tr class=\"tr-balance\"><td colspan=4>Opening Balance</td><td>".$cr_sum."</td><td>".$dr_sum."</td><td>" . convert_opening($opbalance,$optype) . "</td></tr>";
			...
			foreach ($prevbal_q->result() as $row )
			{
				if ($row->entry_items_dc == "D"){
				    ...
				    $dr_sum = float_ops($dr_sum, $row->entry_items_amount, '+');
				}
				else{
					$cur_balance = float_ops($cur_balance, $row->entry_items_amount, '-');
					$cr_sum = float_ops($cr_sum, $row->entry_items_amount, '+');
				}

			}

			/* Show new current total */
			echo "<tr class=\"tr-balance\"><td colspan=4>Opening</td><td>".'Dr '.$dr_sum."</td><td>".'Cr '.$cr_sum."</td><td>" . convert_opening_custom($cur_balance) . "</td></tr>";
		}
