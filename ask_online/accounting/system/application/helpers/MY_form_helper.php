...
if ( ! function_exists('form_dropdown_dc'))
{
	function form_dropdown_dc($name, $selected = NULL, $extra = '')
	{
		...
		$form = '<select name="'.$name.'"'.$extra.' class="dc-dropdown" id="'.$extra.'">';
...
if ( ! function_exists('form_input_ledger'))
{
	function form_input_ledger($name, $selected = NULL, $extra = '', $type = 'all')
	{
		...
		if ($type == 'bankcash')
			...
			$options = $CI->Ledger_model->get_all_ledgers_reconciliation_bankbook();
		else
		...
		$form = '<select name="'.$name.'"'.$extra.' class="ledger-dropdown" id="'.$name.'"'.$extra.'>';
