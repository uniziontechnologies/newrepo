...
class Entry extends Controller {
	...
	function show($entry_type=NULL,$start_date=NULL,$end_date=NULL)
	{
		$this->load->model('Tag_model');
		...
		if ($_POST)
		{
			...
			$data['acc_start_date']=$account_data->fy_start;
			$data['acc_end_date']=$account_data->fy_end;			
			$a=$data['start_date']['value'] = $this->input->post('start_date', TRUE);
			$b=$data['end_date']['value'] = $this->input->post('end_date', TRUE);
			
			if (empty($a)) {
				$a=$data['acc_start_date'];
			}
			if (empty($b)) {
				$b=$data['acc_end_date'];
			}	
			...
			redirect('entry/show/' . $entry_type.'/'.$a.'/'.$b);

		}				

	

		$data['entry_type_page']=$entry_type;
		...
		if (!empty($start_date) || !empty($end_date) ) {
			...
			$account_data = $this->Setting_model->get_current();
			$acc_start_date=$account_data->fy_start;
			$acc_end_date=$account_data->fy_end;

			$acc_start_date = date('Y-m-d', strtotime(str_replace('/', '-', $acc_start_date)));
			$acc_end_date = date('Y-m-d', strtotime(str_replace('/', '-', $acc_end_date)));

			$config['from_date'] = $acc_start_date;
			$config['to_date'] = $acc_end_date;	
		}
		...
		if ($entry_type == "tag") {
			...
			if (!empty($start_date) || !empty($end_date) ){
	            ...
	            $sql="SELECT * FROM `entries` WHERE `entry_type` = '$entry_type_id' AND `date` >= '$start_date' AND `date` <= '$end_date' ORDER BY `date` DESC, `number` DESC LIMIT $page_count,$pagination_counter ";	
	            $query = $this->db->query($sql);
	            $query->result_array();	
	            $entry_q = $query;	

                $sql="SELECT * FROM `entries` WHERE `entry_type` = '$entry_type_id' AND `date` >= '$start_date' AND `date` <= '$end_date' ORDER BY `date` DESC, `number` DESC";
				$query = $this->db->query($sql);
			...
			if (!empty($start_date) || !empty($end_date) ){
	            ...
	            $sql="SELECT * FROM `entries` WHERE `date` >= '$start_date' AND `date` <= '$end_date' ORDER BY `date` DESC, `number` DESC LIMIT $page_count,$pagination_counter ";	
	            $query = $this->db->query($sql);
	            $query->result_array();	
	            $entry_q = $query;	

                $sql="SELECT * FROM `entries` WHERE `date` >= '$start_date' AND `date` <= '$end_date' ORDER BY `date` DESC, `number` DESC";
				$query = $this->db->query($sql);
				$config['total_rows']=$query->num_rows();	 
	...
	function add($entry_type)
	{
		...
		if (!empty($entry_type_id)) {
			$next_entry_number = $this->Entry_model->next_entry_number($entry_type_id);
		}
		else{
			$next_entry_number = "";
		}
		...
		if ($_POST)
		{
			...
			for ($count = 0; $count <= 3; $count++)
			{
				...
				if ($count==1 && ($entry_type=="payment" || $entry_type=="journal") ) {
					break;
				}
				else{
					continue;
	...
	function email($entry_type, $entry_id = 0)
	{
		...
		else
		{
			...
			/* Sending email */
			$this->email->from('', 'Unizion Accounting');
	...
	function dropdown_change()
	{
		...
		$name = $_POST['position'];
		$int = intval(preg_replace('/[^0-9]+/', '', $name), 10);	
		$position=$int;
		...
		if ($selected_value=="D") {
			...
			// $a=form_input_ledger('ledger_id['.$position.']', isset($ledger_id[$position]) ? $ledger_id[$position] : 0, '', $type = 'bankcash');
			$options = $this->Ledger_model->get_all_ledgers_bankcash();
			// var_dump($options);
			$options_view = array('options'=>$options);
			// var_dump($options_view);
			// echo json_encode(array();
			echo json_encode($options_view);

		}
		else{
				// echo form_input_ledger('ledger_id['.$position.']', isset($ledger_id[$position]) ? $ledger_id[$position] : 0,$position);		
			$options = $this->Ledger_model->get_all_ledgers();
			$options_view = array('options'=>$options);
			echo json_encode($options_view);
			// var_dump($options);			
		}
