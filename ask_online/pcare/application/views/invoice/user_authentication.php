...
$(document).ready(function() {
  ...
  $("#authenticate").click( function () {
    ...
    if(($("#username").val() == '' )){
    ...
    }else if(($("#authentication_type").val() == "CREDIT_BILL_AUTHENTICATION" ) && $("#sanctioned_by").val() == '' ){
    
      showDialog('Error','Please Enter Sanctioned By.','error',2);
    }else if(($("#authentication_type").val() == "CREDIT_BILL_AUTHENTICATION" ) && $("#authentication_remarks").val() == '' ){
      ...
      $.post("<?php echo base_url(); ?>index.php/login/user_authentication", $("#user").serialize(),function(data){
         ...
         }else if(($("#authentication_type").val() == "CREDIT_BILL_AUTHENTICATION" ) || ($("#authentication_type").val() == "DISCOUNT_BILL_AUTHENTICATION" )){

          $('#sanc_by_hidden').val($("#sanctioned_by").val());
          $('#auth_remarks_hidden').val($("#authentication_remarks").val());
          
           $('#invoice_form').attr('action',"<?php echo base_url(); ?>index.php/invoice/add_invoice/"+data['user_id']);
          $('#invoice_form').submit();
                }else{
         
          $('#invoice_form').attr('action',"<?php echo base_url(); ?>index.php/invoice/invoice_update/<?php echo $billid;?>/"+data['user_id']);
          $('#invoice_form').submit();
         }
      },"json");
