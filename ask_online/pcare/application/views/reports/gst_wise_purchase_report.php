...
<form name="gst_wise_purchase_report" id="gst_wise_purchase_report" method="post" action="">
...
?>
 ...
 <td>
                ...
                <td>
                        <select name="gst" id="gst" onchange="searchForm();" >
                            <option value="">----------</option>
                            <?php
                                if (!empty($gstInfo)) {

                                    for ($i=0; $i <count($gstInfo) ; $i++) { ?>
                                        <option value="<?php echo $gstInfo[$i][1] ?>" <?php if (!empty($gst) && $gst ==$gstInfo[$i][1] ) {
                                echo "selected";
                            } ?>><?php echo $gstInfo[$i][1]; ?></option>
                                    <?php
                                    }
                    ...
                    <table width="100%" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="font_th"><a href="#">Total GST</a></th>

                                <?php

                                  if (!empty($gstInfo)) {

                                    for ($h=0; $h <count($gstInfo) ; $h++) { ?>

                                      <th class="font_th"><a href="#"><?php echo $gstInfo[$h][1]; ?>% gst</a></th>
                        ...
                        if(!empty($billInfo)){   
                            ...
                            <?php
                                 ...
                                 if(!empty($gstInfo)){   
                                    ...
                                    for ($k=0; $k < count($gstInfo); $k++) { 
                                        
                                        if ($gstInfo[$k][1]==$billInfogst[$i][$k][0][1]) {?>

                                          <td><?php echo number_format((float)$billInfogst[$i][$k][0][0], 2, '.', '');; ?></td>
                                             <?php  $sum1[$k][0]+=$billInfogst[$i][$k][0][0];
                                             $sum1[$k][1]=$gstInfo[$k][1];?>
                                        <?php
                      ...
                      <?php   
                                ...
                                $total_gst=$total_gst+$billInfo[$i][7];
                          ...
                          ?>
                                ...
                                <td ><b><?php echo $total_gst;?></b></td>
                             
                               

                          <?php for ($i=0; $i <count($gstInfo); $i++) { 
                           ?>
                             <td><b><?php if ($gstInfo[$i][1]==$sum1[$i][1]) {
                              echo $sum1[$i][0];
                             }else{echo " - ";}   ?></b></td>   
...
$(document).ready(function() {
  ...
  /*......pagination......*/  
    ...
    $(".next_page").bind('click', function() {
                 ...
                 $("#gst_wise_purchase_report").attr("action","<?php echo base_url(); ?>index.php/reports/gstwisePurchaseReport");
                 $("#gst_wise_purchase_report").submit();
    });
   ...
   function searchForm(){
      ...
      $("#current_page").val('');
      document.gst_wise_purchase_report.action="<?php echo base_url(); ?>index.php/reports/gstwisePurchaseReport";
      document.gst_wise_purchase_report.submit();

    }

    function clearForm(){

        window.location = "<?php echo site_url('reports/gstwisePurchaseReport'); ?>";
        return false;
    ...
    function exportGstPurchase(){
        ...
        $("#current_page").val('');
        document.gst_wise_purchase_report.action="<?php echo base_url(); ?>index.php/reports/gstwisePurchaseReport/export";
        document.gst_wise_purchase_report.submit();
