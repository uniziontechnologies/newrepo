<?php
  ...
  if(!empty($export)){
          ...
          header('Content-Disposition: attachment; filename=cancelled_movement-report.xls');
  }
...
<form name="cancelled_movement_report" id="cancelled_movement_report" method="post" action="">
...
?>
          ...
          <section class="content-header">
            <h1>
                  CANCELLED MOVEMENT REPORT
            </h1>
            <div style="text-align: right">
                <button type="button" data-toggle="modal" data-target="#squarespaceModal" class="btn btn-success" onclick="exportCancelledMovement()" ><img src="<?php echo base_url(); ?>application/assets/dist/img/Excel-icon.png" width="10%" height="10%"> Export to Excel</button>
            </div>
                          ...
                          if(!empty($movement)){            
                            ...
                            for($i=0;$i<count($movement);$i++) {?>
                                <tr>
                                  ...
                                  <td><?php echo $movement[$i][11];?></td>
                                  <td><?php echo $movement[$i][13];?></td>
<?php
if(empty($export)){  
?>
                                  <td class="DONTPrint">
...
$(document).ready(function() {
  ...
  /*......pagination......*/  
    ...
    $(".next_page").bind('click', function() {
                 ...
                 $("#cancelled_movement_report").attr("action","<?php echo base_url(); ?>index.php/reports/cancelled_movement_reports");
                 $("#cancelled_movement_report").submit();
    });
   ...
   function searchForm(){
      ...
      $("#current_page").val('');
      $("#cancelled_movement_report"). removeAttr("target");
      document.cancelled_movement_report.action="<?php echo base_url(); ?>index.php/reports/cancelled_movement_reports";
      document.cancelled_movement_report.submit();

    }

    function clearForm(){

        window.location = "<?php echo site_url('reports/cancelled_movement_reports'); ?>";
        return false;
    ...
    function printPurchase(id) {
            ...
            $("#cancelled_movement_report").attr("target", "_blank");
            document.cancelled_movement_report.action='<?php echo base_url(); ?>index.php/movement/printMovement';
            document.cancelled_movement_report.submit();
    ...
    function exportCancelledMovement(){

        $("#current_page").val('');
        $("#cancelled_movement_report"). removeAttr("target");
        document.cancelled_movement_report.action="<?php echo base_url(); ?>index.php/reports/cancelled_movement_reports/export";
        document.cancelled_movement_report.submit();
