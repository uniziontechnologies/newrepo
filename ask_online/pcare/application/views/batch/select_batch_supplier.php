		           ...
		           <div class="box box-info">
                      ...
                      <div class="box-body">
						...
						<tr>
								...
								<td ><b><?php echo $this->lang->line('supplier'); ?> : <?php echo (!empty($supplier_name))?$supplier_name:'ALL';?></b></td>
																
							</tr>
						</table>
						</div>
				 </div>
				 <div class="main_store">MAIN BRANCH</div>
				 <div class="box box-info">
                      ...
                      <div class="box-body">
					    <table width="100%" class="table table-striped">
							<thead>
								<tr>
									<th width="5%">Sl No</th>
									<th width="15%"><?php echo $this->lang->line('batch');?></th>									
									<th width="20%"><?php echo $this->lang->line('expiry_date');?></th>
...
?>
						...
						<br><br><br>
						<div class="other_branch">OTHER BRANCHES (EXPIRED MEDICINES)</div>
						<table width="100%" class="table table-striped">
							<thead>
								<tr>
									<th width="5%">Sl No</th>
...
<?php 
	    if(!empty($other_branch_batch)){			
		    $j=1;
		    for($i=0;$i<count($other_branch_batch);$i++) { 
?>
		    <tr>
			    <td><?php echo $j++;?></td>
			    <td><?php echo $other_branch_batch[$i][2];?></td>
			    <td><?php echo $other_branch_batch[$i][3];?></td>
				<td><?php echo $other_branch_batch[$i][4];?></td>
				<td><?php echo $other_branch_batch[$i][13];?></td>
				<td>
												
					<?php if((!empty($batchInfo)) && in_array($other_branch_batch[$i][0],$batchInfo)){ ?>

                                <div id='required'>Batch Already Selected</div>
                    <?php }else{ ?>									
							<input id="<?php echo 'batch_id'.$i;?>" type="hidden" value='<?php echo $other_branch_batch[$i][0];?>' />
							<input type="button" name="addbatch" value="Select" id="addbatch<?php echo $i; ?>" class="save_bill btn btn-xs btn-info"  onclick="addbatch('<?php echo $other_branch_batch[$i][0];?>')" onkeydown="ArrowKeynextField(event.keyCode,'<?php echo $i;?>','addbatch','','')" onkeypress="if(event.keyCode== 13){return addbatch('<?php echo $other_branch_batch[$i][0];?>');}"/>
