...
//there is a mismatch in purchase amount in purchase form while item wise discount entered, that discount not deducted from purchase amount
...
$accounting_details =$acc_obj_new->getAccountingAutomisePharmaPurchase();
...
function pharma_purchase($purchase_date_from,$purchase_date_to){
   ...
   $purchase_entry=array();
	 ...
	 $wheredata[5]="purchase_mode = 'Recievings'";
 		...
 		for ($i=0; $i <count($pharma_purchase) ; $i++) { 	
 			...
 			if (!empty($supplier_ledger_id)) {
 						...
 						if (!empty($pharma_purchase[$i][24]) && $pharma_purchase[$i][24]=="YES") {
 							$pharma_purchase[$i][16]=$pharma_purchase[$i][23];
 						}
 						else{
 							$pharma_purchase[$i][16]=$pharma_purchase[$i][16];
 						}
						...
						if ($pharma_purchase[$i][14]=="CASH") {
							...
							//cash amount
							 ...
							 $entry_items['entry_id']=$entry_id;
							 $entry_items['amount']=$pharma_purchase[$i][16];	//amount paid cash	
						...
						if ( $pharma_purchase[$i][14]=="CREDIT CARD" ) {
                            ...
                            //journal entry purchase medicine to supplier account
							 ...
							 $entry_items['entry_id']=$entry_id_1[0];
							 $entry_items['amount']=$pharma_purchase[$i][16];//card amount given	
						...
						if ( $pharma_purchase[$i][14]=="CHEQUE" ) {
					        ...
					        /********** Purchase Amount  *************/
							 ...
							 $entry_items['entry_id']=$entry_id;
							 $entry_items['amount']=$pharma_purchase[$i][16];//cheque amount given	
						...
						if ( $pharma_purchase[$i][14]=="UPI" ) {
                            ...
                            //journal entry purchase medicine to supplier account
							 ...
							 $entry_items['entry_id']=$entry_id_1[0];
							 $entry_items['amount']=$pharma_purchase[$i][16];//upi amount given	
						...
						if ( $pharma_purchase[$i][14]=="NEFT" ) {
                            ...
                            //journal entry purchase medicine to supplier account
							 ...
							 $entry_items['entry_id']=$entry_id_1[0];
							 $entry_items['amount']=$pharma_purchase[$i][4];//neft amount given	
							 ...
							 $entry_items['entry_id']=$entry_id_1[0];
							 $entry_items['amount']=$pharma_purchase[$i][16];//neft amount given	
							...
							if ( $pharma_purchase[$i][14]=="NEFT" && $pharma_purchase[$i][15]==1 ) {
						...
						// var_dump($entry_id);//comment while testing
						...
						if (!empty($entries[$i])) {
							...
							if ($entryItems[$i] != ($entries[$i][0][5]+$entries[$i][0][6])) {
								...
								if (!empty($diff[$i])) {
									$amount_to_update = ($pharma_purchase[$i][16]-$diff[$i]);
									// var_dump($entries[$i][0][0],$diff[$i]);
									$acc_obj->updatePurchaseAmount($entry_id,$amount_to_update,84);
								}
