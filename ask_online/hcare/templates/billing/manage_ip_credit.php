   ...
   function submitform(action,id){
		...
		if(action == "PRINT"){
  		 ...
  		 if(a==true)
   			...
   			document.manage_bill.action="../../lib/controllers/centralController.php?module=Billing&sub_module=Manage_IP_Credits";
		}
		document.manage_bill.submit();
		return true;
   }
    function pay_now(billid){
	     
	      tb_show('IP CREDIT PAYMENT FORM',"../../lib/controllers/centralController.php?module=Billing&sub_module=ip_credit_payment_form&billno="+billid);
   }
   function payAll(){
 
 	var isChecked = false;
	var total=0;var amt=0;
		with (document.manage_bill) {
			for (i in elements) {
		...
		//var v=confirm("Do you want to pay selected bills");
			if(v==true){
					document.manage_bill.action="../../lib/controllers/centralController.php?module=Billing&sub_module=Add_Multiple_Credit_IP";
			}else{
			return false;
			}
			document.manage_bill.submit();
			...
			$j=1;
				for($i=0;$i<count($billInfo);$i++) {?>
					<tr>
						...
						<td>
							<a href="#" class="btn btn-success" onClick="pay_now('<?php echo $billInfo[$i][0];?>');">PAY NOW</a>
					     ...
					     <?php 
						  ...
						  if($user_type_logged_in=='ADMIN' || $user_type_logged_in=='ADMIN+DOCTOR'){?>
							...
							<td><input type="checkbox"   value="<?php echo $billInfo[$i][0]."#".$billInfo[$i][24];?>" name="paybill[]" id="paybill<?php echo $i;?>" title="<?php echo "Bill No: ".$billInfo[$i][0];?>">
				                        <input type="hidden" value="<?php echo $billInfo[$i][24];?>" name="amount[]" id="amount<?php echo $i;?>" >			
							</td>
								<?php 
