   ...
   function submitform(action,id){
		...
		if(document.manage_bill.from_time.value == "" && document.manage_bill.to_time.value != ""){
		  ...
		  return false;
   			...
   			document.manage_bill.action="../../lib/controllers/centralController.php?module=Report&sub_module=observation_payment_report";
		}
   ...
   function download_pdf(){
   		$('.hide_div').show();
				$(".table2excel").table2excel({
					...
					filename: "Observation Payment Report",
					fileext: ".xls",
					exclude_img: true,
					exclude_links: true,
					exclude_inputs: true
	...
	<section class="content">
	 <div class="DONTPrint">				 
	   <div class="box box-info">
				...
				<table class="table table-striped">
							...
							<?php echo $lang_user; ?><?php echo $lang_type; ?> </td>
								...
								<?php } ?>
											...
											<?php for($i=0;$i<count($user_type);$i++){ 
													...
													if(!empty($post['user_type']) && $post['user_type']==$user_type[$i][1]) { ?>
							...
							<?php echo $lang_user; ?> </td>
								...
								<tr>	
									...
									<td id="noborder" colspan="7" align="center">
									&nbsp;&nbsp;
			...
			<h4 ><?php echo 'OBSERVATION PAYMENT REPORT '; ?>From <?php echo $post['from_date']. " ".$post['from_time'];?> To <?php echo $post['to_date']." ".$post['to_time'];?>
			...
			<div class="box box-info">
				...
				<thead>
						<tr class="hide_div"><td><h4><?php echo 'OBSERVATION PAYMENT REPORT '; ?> From <?php if (!empty($post['from_date'])) {
			...
			$j=1;
				for($i=0;$i<count($billInfo);$i++) {?>
					<tr>
						...
						<td><?php echo date("d-m-Y h:i A",strtotime($billInfo[$i][3]));?></td>
						<td><?php echo 'OP';?></td>
						<td><?php echo $billInfo[$i][1];?></td>
						<td><?php echo $billInfo[$i][19];?></td>
						<td><?php echo $billInfo[$i][10]+$billInfo[$i][11]+$billInfo[$i][12]+$billInfo[$i][26];?></td>
						<td><?php echo $billInfo[$i][9];?>
						<td><?php echo $billInfo[$i][10];?>
						<td><?php echo $billInfo[$i][11];?>
						<td><?php echo $billInfo[$i][26];?>
						<td><?php echo $billInfo[$i][12];?>
						<td class="DONTPrint"><?php echo $billInfo[$i][25];?></td>
						<td class="DONTPrint"><a href="#" class="btn btn-info btn-flat" onclick="print_bill('<?php echo $billInfo[$i][0]; ?>');"><i class="fa fa-print"></i></a></td>
							
					</tr>
					<tr>
		<?php
		$total +=($billInfo[$i][10]+$billInfo[$i][11]+$billInfo[$i][12]+$billInfo[$i][26]);
		$totalcash +=$billInfo[$i][10];	
		$totalcard +=$billInfo[$i][11];	
		$totalcheque +=$billInfo[$i][12];	
		$totalupi +=$billInfo[$i][26];			
...
?>				
	 ...
	 <input type="hidden" name="page_name" id="observation_payment_report" value="observation_payment_report" />
