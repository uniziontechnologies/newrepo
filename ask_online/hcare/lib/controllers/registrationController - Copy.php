...
class RegistrationController {
	...
	function viewPage($sub_module,$postArr='',$getArr='',$message=''){
			...
			switch($sub_module){
				...
				case 'patient_re_registeration'	:   $postArr = array(); $postArr['paction']="REREG_FORM"; 
				case 'patient_registeration'	:   
													...
													$post['card_expiry']=date('d-m-Y', "+$y years");
													...
													$no_of_year=$validity_info[0][5];
													 ...
													 // exit;
													 $post['card_expiry'] =date('d-m-Y', "+$m months");
												}
				...
				case 'search_op_patient'    :           $com_obj = new CommonFunctions();
														...
														$emp_obj= new Employee();
														  ...
														  $form_creator ->popArr['patient_info']=$reg_obj->getOPPatientInfo('',$selectCondition,'b.visit_date','desc');
...
// var_dump(expression)
   if (!empty($patientInfo)) {
	   ...
	   for ($i=0; $i <count($patientInfo) ; $i++) { 
			...
			$casesheetInfo[$i][0]=$dr_obj-> get_ip_Present_complaint('', $where,"id",'desc');
			$casesheetInfo[$i][1]=$dr_obj-> get_ip_Prov_diagnosis('', $where,"id",'desc');
			$casesheetInfo[$i][2]=$dr_obj-> get_ip_Procedure_presc('', $where,"id",'desc');
			$casesheetInfo[$i][3]=$dr_obj->get_ip_Labtest_presc('', $where,"id",'desc');
			$casesheetInfo[$i][4]=$dr_obj->get_ip_Medicine_presc('', $where,"id",'desc');
			$casesheetInfo[$i][8]=$dr_obj->get_ip_PastHistory('', $where,"id",'desc');
			$casesheetInfo[$i][5]=$patient_id;
			$casesheetInfo[$i][9]=$dr_obj-> get_ip_PhysicalExamination('', $where,"id",'desc');
			$casesheetInfo[$i][11]=$dr_obj-> get_ip_Radiology_presc('', $where,"id",'desc');
...
function processOPNo_ref($post,$get = null){
	...
	function doc_fee_change($post){
		...
		if (!empty($post['docid'])) {
			...
			$ref_doc_id = $post['ref_doc_id'];
						...
						$data=$docfee.":".$regfee.":".$cardfee.":".$post['ref_balance'].":".$post['referal_type'];
