...
class Billing{
	...
	function addCustomer($post){
		...
		$field_names=array("id","name","age","gender","place","contact_no","refferal_info","prefix","doc_id","doc_name");
		$field_data=array("",$post['name'],$age,$post['gender'],$post['place'],$post['contact_no'],$post['refferal_info'],$post['prefix'],$post['doctor_direct_ID'],$post['doctor_direct']);
	...
	function addCredit($post){
	    ...
	    $total_amount = $post['amount_paid'] + $post['card_amount']+$post['upi_amount'];
			 
		$time=date("H:i:s");
		$field_names=array("id","bill_no","bill_date","bill_time","amount","user_id","payment_mode","amount_paid","card_amount","upi_amount");
		$field_data=array("",$post['billid'],date('Y-m-d',strtotime($post['new_date']))." ".$time,date("h:i a"),$total_amount,$_SESSION['user_id'],$post['payment_mode'],$post['amount_paid'],$post['card_amount'],$post['upi_amount']);
	...
	function updateCustomer($post){
		...
		if($post['paction'] == 'EDIT_PATIENT_INFO'){
			$age=$post['age'];
		...
		$field_names=array("name","age","gender","place","contact_no","refferal_info","prefix","doc_id","doc_name");
		$field_data=array($post['name'],$age,$post['gender'],$post['place'],$post['contact_no'],$post['refferal_info'],$post['prefix'],$post['doctor_direct_ID'],$post['doctor_direct']);
	...
	function addtoBill($post){ 
		...
		if(!empty($post['upi_amount'])){
			$upi_amount=$post['upi_amount'];
		}else $upi_amount=0;
		...
		$amount_paid=$post['amount_paid']+$credit_card+$ins_deduction+$upi_amount;
		...
		$field_names=array("id","type","ref_no","total_amount","dr_disc","net_total","paid_with","cash","credit","credit_card","insurance","bill_date","remarks","user_id","opno","update_history","sanctioned_by","sanctioned_remarks","category_id","member_id","email","bill_type","upi","bill_user_type","bill_entered_user_type_id");
		$field_data=array("",$post['type'],$post['id'],$post['total_amount'],$post['dr_disc'],$post['net_amount'],$post['payment_mode'],$post['amount_paid'],$credit,$credit_card,$ins_deduction,date("Y-m-d",strtotime($post['date']))." ".$time,$post['remarks'],$_SESSION['user_id'],$post['opno'],$update_history,$sanc_by,$auth_remarks,$post['patient_cat_id'],$post['patient_member_id'],$post['email'],$post['bill_type'],$upi_amount,$user_type,$user_type_id);
	...
	function add_advance_payment($post){
	    ...
	    $field_names=array("id","ipno","payment_mode","cash","card_amount","cheque_no","date","remarks","user_id","upi_amount");
	    $field_data=array("",$post["id"],$post["payment_mode"],$post["advance_amount"],$post["card_amount"],$post["cheque_no"],$post['curr_date_time'],$post['remarks'],$_SESSION['user_id'],$post["upi_amount"]);
	...
	if($post['payment_mode'] == "CREDIT") {
	  ...
	  $credit_amount=$post['balance']-$post['final_amount'];
	   ...
	   $field_names=array( "bill_date","disc_type", "disc_amt", "net_amount", "payment_mode", "amount", "card_amount","balance","cheque_amt","cheque_no", "remarks","user_id","update_history","auth_sanc_by","auth_remarks","upi_amount");
	   
	   $field_data=array($post['curr_date'],$post['disc_type'], $post['discount'], $post['balance'], $post['payment_mode'], $post['final_amount'], $post['card_amount'],$credit_amount,$post['cheque_amount'],$post['cheque_no'], $post['remarks'],$_SESSION['user_id'],$update_history,$post['auth_sanc_by'],$post['auth_remarks'],$post['upi_amount']);
	...
	function updateBill($post){
		...
		$amount_paid=$post['amount_paid']+$credit_card+$upi_amount+$ins_deduction;
		$credit=$post['net_amount']-$amount_paid;
	
		$field_names=array("type","ref_no","total_amount","dr_disc","net_total","paid_with","cash","credit","credit_card","insurance","bill_date","remarks","update_history","upi");
		$field_data=array($post['type'],$post['id'],$post['total_amount'],$post['dr_disc'],$post['net_amount'],$post['payment_mode'],$post['amount_paid'],$credit,$credit_card,$ins_deduction,date("Y-m-d",strtotime($post['date'])),$post['remarks'],$update_history,$upi_amount);
			        ...
			        if(!empty($row['update_history'])){
					  ...
					  $update_history .=$user_name.":".$datetime."<br>";
			           ...
			           $arrList[$i][17]= $update_history;
						...
						$total_amount_paid=$row['amount_paid']+$row['amount']+$row['card_amount']+$row['cheque_amt']+$row['upi_amount'];
	...
	public function getfullCreditInfo($wheredata){   
				...
				if(mysqli_num_rows($result)>0){
						...
						while($row=mysqli_fetch_array($result)){
                               ...
                               $arrList[$i][$k++]=$this->dbConnection->idToValue("hcare_bill_payments","upi_amount","id",$row[15]);
                             
                             // $arrList[$i][$k++]=$a[$i]=$this->dbConnection->idToValue("hcare_bill_payments","cancellation_details","id",$row[15]);
                               $arrList[$i][$k++]=$this->dbConnection->idToValue("hcare_bill","bill_user_type","id",$row[0]);
	...
	function getBillItemPatient($wheredata = null,$groupby= null ){
	  ...
	  //new
	  $arrFieldList[5]="a.net_total";
	  $arrFieldList[6]="a.cash";
	  $arrFieldList[7]="a.credit";
	  $arrFieldList[8]="a.credit_card";
	  $arrFieldList[9]="a.insurance";

	  	//category id for labour charges
	// $arrFieldList[10]="b.category_id";
	// $arrFieldList[11]="b.bill_id";
				...
				if(mysqli_num_rows($result)>0){
						...
						while($row=mysqli_fetch_array($result)){
						  ...
						  if($row['type'] == "C") {
							...
							$arrList[$i][8]= $row['cash']+$row['credit_card']+$row['insurance'];
							$arrList[$i][9]=$row["credit"];
							$arrList[$i][10] = $this->getBillItemLabCredit($row['id']);
	...
	$result=$this->dbConnection->executeQuery($query);
				...
				if(mysqli_num_rows($result)>0){
						...
						while($row=mysqli_fetch_array($result)){
							...
							$arrList[$i][3]= $row['cash']+$row['credit_card']+$row['insurance']+$row['upi'];
	...
	function getBillTheatreProcedure($wheredata = null,$groupby= null ){
	  ...
	  $arrFieldList[13]="b.assistant_fee1";
	  $arrFieldList[14]="b.assistant_fee2";
				...
				if(mysqli_num_rows($result)>0){
						...
						while($row=mysqli_fetch_array($result)){
						 ...
						 $arrList[$i][7] = $row['net_total'];			  	
							...
							$arrList[$i][9]=$row["credit"];
							$arrList[$i][10] = $this->getBillItemLabCredit($row['id']);
							$arrList[$i][11] =$row["assistant_fee1"];
							$arrList[$i][12] =$row["assistant_fee2"];
						 $i++;
	...
	$result=$this->dbConnection->executeQuery($query);
				...
				if(mysqli_num_rows($result)>0){
						...
						while($row=mysqli_fetch_array($result)){
						 ...
						 $arrList[$i][13] = $this->getBillItemLabCredit($row['id']);
						 $i++;
	...
	function getBillInfoAllField($wheredata = null ,$orderbyfield = null,$oderby = null,$credit_status = false){
		$arrList=array();
				...
				if(mysqli_num_rows($result)>0){
									...
									$doc_id=$this->dbConnection->idToValue("hcare_ip_info","doc_id","id",$row[2]);
                                        ...
                                        if($discharge_date !='' && $discharge_date !=="0000-00-00"){
                                           ...
                                           if($payment_mode =="CREDIT CARD"){
                                           	$ip_card_amount=$row['credit'];

                                           }elseif($payment_mode =="UPI"){
                                           	$ip_upi_amount=$row['credit'];

                                           }else{
                                           	$ip_cash_amount=$row['credit'];;
                                           }
						...
						$sql1 = "select sum(amount_paid),sum(card_amount),sum(upi_amount) from `hcare_bill_payments` where `bill_no`='".$row[0]."' and status=0";
														...
														if(mysqli_num_rows($result1) > 0){
															...
															while($row1=mysqli_fetch_array($result1)){
																...
																$arrList[$i][39]=$row1['sum(upi_amount)'];
								...
								$arrList[$i][40]=$row[37]+$ip_upi_amount;

								$arrList[$i][41]=0;

								 $sql_new = "SELECT `result` FROM `hcare_lab_result_entry` WHERE `bill_no`='".$row[0]."' AND result_id = (SELECT MAX(result_id) FROM hcare_lab_result_entry WHERE bill_no = '".$row[0]."') AND tid !=0 AND catid !=0";

								$result_new=$this->dbConnection->executeQuery($sql_new);
														...
														if(mysqli_num_rows($result_new) > 0){
															...
															while($row_new=mysqli_fetch_array($result_new)){
																...
																if (isset($row_new['result'])) {
																	

																	for ($d=0; $d < count($row_new['result']) ; $d++) { 
																		// var_dump($row_new['result'][$d]);
		...
		if(mysqli_num_rows($result) > 0){
			...
			if($data){
				...
				if($row['type'] == "OP" || $row['type'] == "IP") {
					...
					$department_id=$this->dbConnection->idToValue("hcare_emp_job_info","department_id","emp_id",$doc_id);
									
					$arrList[$i][8]=$this->dbConnection->idToValue("hcare_emp_info","title","id",$doc_id)." ".$this->dbConnection->idToValue("hcare_emp_info","first_name","id",$doc_id)." ".$this->dbConnection->idToValue("hcare_emp_info","last_name","id",$doc_id);
					$arrList[$i][49]=$this->dbConnection->idToValue("hcare_emp_job_info","qualification","emp_id",$doc_id)." ".$this->dbConnection->idToValue("hcare_hospital_department","department","id",$department_id);
					...
					$doc_id_direct = $this->dbConnection->idToValue("hcare_direct_customer","doc_id","id",$row['ref_no']);

					$department_id=$this->dbConnection->idToValue("hcare_emp_job_info","department_id","emp_id",$doc_id_direct);

					$doc_name_direct = $this->dbConnection->idToValue("hcare_direct_customer","doc_name","id",$row['ref_no']);

					if (!empty($doc_name_direct)) {
						$arrList[$i][8]=$doc_name_direct;
					}
					else{
						$arrList[$i][8]=$this->dbConnection->idToValue("hcare_emp_info","title","id",$doc_id_direct)." ".$this->dbConnection->idToValue("hcare_emp_info","first_name","id",$doc_id_direct)." ".$this->dbConnection->idToValue("hcare_emp_info","last_name","id",$doc_id_direct);
					}

					$arrList[$i][49]=$this->dbConnection->idToValue("hcare_emp_job_info","qualification","emp_id",$doc_id_direct)." ".$this->dbConnection->idToValue("hcare_hospital_department","department","id",$department_id);

					$arrList[$i][50]=$row['email'];

					$arrList[$i][53] = $this->dbConnection->idToValue("hcare_direct_customer","refferal_info","id",$row['ref_no']);

					$arrList[$i][54] = $this->dbConnection->idToValue("hcare_direct_customer","doc_id","id",$row['ref_no']);

// var_dump($arrList[$i][53]);
			        ...
			        //split updation history
			                                        ...
			                                        $arrList[$i][27]= $update_history;
			                                         ...
			                                         if(!empty($history_split)){
														...
														$sql1 = "select sum(amount_paid),sum(card_amount),sum(upi_amount) from `hcare_bill_payments` where `bill_no`='".$row['id']."' and status=0";
														...
														if(mysqli_num_rows($result1) > 0){
															...
															while($row1=mysqli_fetch_array($result1)){
																...
																$arrList[$i][56]=$row1['sum(upi_amount)'];
														...
														$arrList[$i][51]=$this->dbConnection->idToValue("hcare_bill_items","type","bill_id",$row['id']);
														// var_dump($row['user_id']);
														$arrList[$i][52]=$this->dbConnection->idToValue("hcare_users","user_type","id",$row['user_id']);
														$arrList[$i][55]=$row['upi']+$ip_upi_amount;
														$arrList[$i][57]=$row['bill_user_type'];
	...
	function getBillInfo2($wheredata = null ,$selectdata=null,$orderbyfield = 'id',$oderby = 'asc',$credit = false,$limit=null ){
		...
		if(mysqli_num_rows($result) > 0){
			...
			if($data){
				...
				$arrList[$i][13]=$row['cash']+$creditPayment+$ip_cash_amount;
				$arrList[$i][15]=$row['credit_card']+$ip_card_amount;
				$arrList[$i][16]=$row['upi']+$ip_upi_amount;
				$i++;
		...
		if(mysqli_num_rows($result) > 0){
			...
			while($row=$result -> fetch_assoc()){
				...
				if ($row['package_id'] > 0) {
					...
					$wheredata_mul[1] = "package_id = '".$row['package_id']."'";
					$wheredata_mul[2] = "test_id = '".$row['test_id']."'";

					$arrList[$i][11]=$db_function->getIdToValueMultiple("order_no",$wheredata_mul,"hcare_healthpackage_elements");
			...
			while($row=$result -> fetch_assoc()){
				...
				if(!empty($creditInfo)){
					...
					for($k=0;$k<count($creditInfo);$k++){
						...
						$credit +=($creditInfo[$k][2]+$creditInfo[$k][5]+$creditInfo[$k][10]);
					}
				 ...
				 /*patient recod ip pharmacy bill info balance mismatch because of the following line
				   ...
				   $rem_balance=0;
					...
					$rem_balance=$row['balance']-$credit; 
				                ...
				                $balance=$row['balance'];
								$new_amount=$amount_paid+$credit+$card_amt+$upi_amt;
								// var_dump($balance,$new_amount);
                                $billInfo[$i][1]= $row['id'];
	...
	function add_ip_credit_payment($post){
		...
		$field_names=array("id","billid","ipno","opno","payment_mode","amount_paid","card_amount","cheque_no","bill_date","remarks","user_id","upi_amount");
		$field_data=array("",$post['billid'],$post['ipno'],$post['opno'],$post['payment_mode'],$post['cash_amount'],$post['card_amount'],$post['cheque_no'],$post['bill_date'],$post['remarks'],$_SESSION['user_id'],$post['upi_amount']);
	...
	function updateBillAmounts($post){
		...
		$field_names=array("total_amount","dr_disc","net_total","cash","credit","credit_card","insurance","update_history","upi");
		$field_data=array($post['total_amount'],$post['dr_disc'],$post['net_amount'],$post['amount_paid'],$credit,$credit_card,$ins_deduction,$update_history,$upi_amount);
	...
	function addObservationBill($post){
	  ...
	  $time=date("H:i:s");
	    ...
	    $field_names=array("id","opno","visit_id","bill_date", "total_amount","amount_paid","disc_type","disc_amt","balance","payment_mode","cash_amount","card_amount","cheque_amt","cheque_no","remarks","user_id","update_history","cancellation_details","status","upi_amount");
	   
	    $field_data=array("",$post['opno'],$post['visit_id'],date("Y-m-d",strtotime($post['dod']))." ".$time,$post['total_amount'], $post['amount_paid'],$post['disc_type'],$post['discount'],$post['balance'],$post['payment_mode'],$post['cash_amount'],$post['card_amount'],$post['cheque_amount'],$post['cheque_no'],$post['remarks'],$user_id,$update_history,'',0,$post['upi_amount']);
	...
	if(mysqli_num_rows($result) > 0){			
		while($row=$result -> fetch_assoc()){
			...
			if ($row["b_id"]) {
				...
				$arrList[$i][3] = $row["amount_paid"]+$row["card_amount"]+$row["upi_amount"];

				$bill_no_old = $bill_no_now;
				$bill_no_now = $row["bill_no"];
				
				if($new_bal == 0 && $bill_no_old == $bill_no_now){
					$new_bal = $row["credit"]-($row["amount_paid"]+$row["card_amount"]+$row["upi_amount"]);
				}
				else if($bill_no_old == $bill_no_now){			
					$new_bal -= ($row["amount_paid"]+$row["card_amount"]+$row["upi_amount"]);
				}
				else{
					$new_bal = $row["credit"]-($row["amount_paid"]+$row["card_amount"]+$row["upi_amount"]);
				}
				$arrList[$i][4] = $new_bal;
				$paid_tot += ($row["amount_paid"]+$row["card_amount"]+$row["upi_amount"]);
				$balance_tot -= ($row["amount_paid"]+$row["card_amount"]+$row["upi_amount"]);
			}
			$i++;
		}
	}
...
function final_bill_advance_details($ip_no){
	...
	$arrFieldList[6] = "`upi_amount`";

	$selectConditions[0] = "ipno='".$ip_no."'";
	$selectConditions[1] = "status=0";
	...
	if(mysqli_num_rows($result) > 0){			
		while($row=$result -> fetch_assoc()){
			...
			$arrList[$i][3] = $row["cash"]+$row["card_amount"]+$row["upi_amount"];
			$paid_tot += ($row["cash"]+$row["card_amount"]+$row["upi_amount"]);
			$i++;
		}
	}
...
function final_bill_pharma_details($ip_no){
	...
	$arrFieldList[8] = "`upi_amt`";

	$selectConditions[0] = "ip_no='".$ip_no."'";
	$selectConditions[1] = "status=0";
	// new
		$selectConditions[2] = "sales_mode='Sales'";
	...
	if(mysqli_num_rows($result) > 0){			
		while($row=$result -> fetch_assoc()){
			...
			$arrList[$i][1] = $row["bill_date"];
			$arrList[$i][2] = $row["net_total"];
			$arrList[$i][3] = $row["checque_amt"]+$row["card_amt"]+$row["amount_paid"]+$row["upi_amt"];
			$arrList[$i][4] = $row["balance"];
			$net_tot += $row["net_total"];
			$paid_tot += ($row["checque_amt"]+$row["card_amt"]+$row["amount_paid"]+$row["upi_amt"]);
...
function final_credit_bill_pharma_details($ip_no){
	...
	$arrFieldList[12] = "b.`bill_no`";
	$arrFieldList[13] = "b.`upi_amt`";
		$arrFieldList[14] = "b.`card_amt`";
	...
	$selectConditions[1] = "a.status=0";
	$selectConditions[2] = "b.status=0";
	// new
		$selectConditions[3] = "a.sales_mode='Sales'";
	...
	if(mysqli_num_rows($result) > 0){			
		while($row=$result -> fetch_assoc()){
			...
			if($row["b_id"]){			
				...
				$arrList[$i][1] = $row["date"];
				$arrList[$i][2] = $row["bill_no"];
				$arrList[$i][3] = ($row["amount"]+$row["card_amt"]+$row["upi_amt"]);

				$bill_no_old = $bill_no_now;
				$bill_no_now = $row["bill_no"];
				
				if($new_bal == 0 && $bill_no_old == $bill_no_now){
					$new_bal = $row["balance"]-($row["amount"]+$row["card_amt"]+$row["upi_amt"]);
				}
				else if($bill_no_old == $bill_no_now){			
					$new_bal -= ($row["amount"]+$row["card_amt"]+$row["upi_amt"]);
				}
				else{
					$new_bal = $row["balance"]-($row["amount"]+$row["card_amt"]+$row["upi_amt"]);
				}
				...
				$arrList[$i][4] = $new_bal;
				$paid_tot += ($row["amount"]+$row["card_amt"]+$row["upi_amt"]);
				$balance_tot -= ($row["amount"]+$row["card_amt"]+$row["upi_amt"]);
			}
			$i++;
		}
	}
...
return $arrList;
	...
	function getIPBillConsolidated($wheredata = null){
	   ...
	   $query=$this->dbConnection->BuiltQuery('hcare_ip_bill',$arrFieldList,$wheredata,'id','asc');	
		...
		if(mysqli_num_rows($result)>0){
	            ...
	            $arrList[$i][5]=$row["sum(upi_amount)"];
	   ...
	   $query=$this->dbConnection->BuiltQuery('hcare_advance_payments',$arrFieldList,$wheredata,'id','asc');	
		...
		if(mysqli_num_rows($result)>0){
						...
						while($row=mysqli_fetch_array($result)){
						  ...
						  $arrList[$i][0]=$row['sum(cash)'];
						  $arrList[$i][1]=$row['sum(card_amount)'];
						  $arrList[$i][2]=$row['sum(upi_amount)'];
	...
	public function getDirectCustomers($wheredata){
	
		$arrList=array();
		$i=0;
		$query=$this->dbConnection->BuiltQuery('hcare_direct_customer','',$wheredata,'id','asc');	
		$result=$this->dbConnection->executeQuery($query);
		...
		if(mysqli_num_rows($result) > 0){
			...
			while($row=$result -> fetch_assoc()){
				...
				$arrList[$i][8]=$row['online_user_id'];
				$arrList[$i][9]=$row['status'];
				$arrList[$i][10]=$row['doc_id'];
				$arrList[$i][10]=$row['doc_name'];
...
function getBillCountNew($whereField){

	$arrList = 0;

	$query = "select count(id) from hcare_bill ";
