...
$accounting_details =$acc_obj_new->getAccountingAutomisePharmaSalesReturn();
...
if ($pharma_sales[0]>0) {
	...
	if ($accounting_details[0][17]==1) {
		...
		$entry_id = accounting_insertion_entry(($pharma_sales[5]+$pharma_sales[2]),$sales_date);
...
function pharma_sales($sales_date_from,$sales_date_to){
 ...
 //pharmacy collection
	 ...
	 $wheredata[3]="sales_mode = 'Return'";
	  ...
	  $total_pharma_collection[1]=number_format((float)($pharma_collection[0][4]-$pharma_round_0ff_collection-$pharma_gst_collection), 2, '.', '') ;
...
function pharma_sales_gst($sales_date_from,$sales_date_to){
	...
	$pharma_obj=new pharmaFunctions();
	 ...
	 if (!empty($sales_date_from) && !empty($sales_date_to)) {
	 		...
	 		$sales_gst_per =$pharma_obj->getSalesBillGstReturn($sales_date_from,$sales_date_to);
	 		...
	 		if (!empty($sales_gst_per)) {
	 			...
	 			for ($i=0; $i <count($sales_gst_per) ; $i++) { 
	 				...
	 				$pharma_sales_gst[$i] = $pharma_obj->getSalesBillGstAmountReturn($sales_gst_per[$i],$sales_date_from,$sales_date_to);
	 	

	 				$total_gst[] = ($pharma_sales_gst[$i][0][0] + $pharma_sales_gst[$i][0][1]);

	 				// var_dump($pharma_sales_gst[$i]);
